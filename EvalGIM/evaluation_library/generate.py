...
class DiffusersPipeline(ImageGenerationPipeline):
    def __init__(self, model_id: str, cfg_scale: float, device: int | str = "balanced"):
        ...
        self.pipeline = DiffusionPipeline.from_pretrained(
            model_id, torch_dtype=torch.float16, device_map=device #, safety_checker=None
        )
...
#         json.dump(ds, f, indent=4)
    ...
    ds = []
    # output_path_new = Path(args.image_output_folder)
    json_path = Path(args.json_file)
    with open(json_path, 'r') as file:
        dataJson = json.load(file)
        for data_json in dataJson:
            cur_image_path = data_json['image_path']
            new_image_path = os.path.join(images_path, cur_image_path)
            data_json['image_path'] = new_image_path
            ds.append(data_json)
...
def parse_args():
    ...
    # parser.add_argument("--image_output_folder", type=str, default=None)
